...
export class CalendarService {
  ...
  async getInterviewEvents(
    ...
    const cancelledEvents = hrEvents.filter(
      (event) => event.status === 'cancelled' && !event.summary,
    );

    const eventsForGroq = hrEvents.filter(
      (event) => !(event.status === 'cancelled' && !event.summary),
    );
    ...
    this.logger.log(
      `Fetched ${events.length} events, ${hrEvents.length} HR events, ${parsed.length} interview events`,
    );
  ...
  /**
   * Works out whether an event is fresh ("Active") or has been edited
   * since creation ("Updated"), using Google's own `created` and
   * `updated` timestamps.
   */
      ...
      try {
        ...
        if (response.ok) {
          ...
          try {
            ...
            this.logger.warn('Groq batch response does not contain an events array.');
            return events.map(() => ({}));
          } catch {
        ...
        if (response.status === 429 && attempt < MAX_GROQ_RETRIES) {
          const errorText = await response.text();

          this.logger.warn(
            `Groq rate limit reached. Batch retry ${attempt + 1}/${MAX_GROQ_RETRIES}.`,
          );
